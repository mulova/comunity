...
namespace commons {
	...
	public class WeakHashSet<T> where T:class {
		private WeakValueDictionary<IntWrapper, T> store1 = new WeakValueDictionary<IntWrapper, T>();
		private WeakValueDictionary<IntWrapper, List<T>> store2 = new WeakValueDictionary<IntWrapper, List<T>>();
		
		public int Count {
			get {
				int count = store1.Count;
				foreach (KeyValuePair<IntWrapper, List<T>> slot in store2) {
		...
		public bool Add(T obj) {
			IntWrapper key = IntWrapper.ValueOf(obj.GetHashCode());
