...
namespace build
{
    public class BuildSettingTab : EditorTab
    {
        ...
        public override void OnEnable()
        {
            ...
            Reset();
            string assetDigest = EditorPrefs.GetString(DIGEST_ASSET_PREF);
            if (!string.IsNullOrEmpty(assetDigest))
            {
                digestAsset = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(assetDigest), typeof(TextAsset)) as TextAsset;
            }
        ...
        {
            if (EditorUI.DrawHeader("Etc"))
            {
                EditorUI.BeginContents();
                if (EditorGUIUtil.ObjectField<TextAsset>("Asset for Digest", ref digestAsset, false)&&digestAsset != null)
                {
                    EditorPrefs.SetString(DIGEST_ASSET_PREF, AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(digestAsset)));
                }
                EditorUI.EndContents();
            }
        }
            ...
            if (EditorUI.DrawHeader("Patch"))
            {
                ...
                if (GUILayout.Button("Generate"))
                {
                    ...
                    foreach (KeyValuePair<string, FileInfo> pair in dstFiles)
                    {
                        ...
                        {
                            string srcDigest = HashFunction.Compute(srcFiles.Get(pair.Key).Open(FileMode.Open));
                            string dstDigest = HashFunction.Compute(pair.Value.Open(FileMode.Open));
        ...
        private string GenerateCode()
        {
            ...
            Append(str, BuildConfig.KEY_VERSION, version);
            Append(str, BuildConfig.KEY_VERSION_CODE, versionCode);
            Append(str, BuildConfig.KEY_RES_VER, clientVersion);
            Append(str, BuildConfig.KEY_BUILD_TIME, System.DateTime.Now.Ticks);
            Append(str, BuildConfig.KEY_CDN, cdn);
            if (digestAsset != null)
            {
                Append(str, BuildConfig.KEY_DIGEST, BuildConfig.GenerateDigest(digestAsset.bytes));
            }
            return str.ToString();
        }
