...
editor_project='Assembly-CSharp-Editor.csproj'
runtime_project='Assembly-CSharp.csproj'
...
def run_mod( postbuild_cmd, define=None):
    ...
    tree = ET.parse(editor_project)
    root = tree.getroot()
    set_assembly_name(root, 'effect-Editor')
    if postbuild_cmd:
        set_custom_command(root, '../copy_dll.sh')
    else:
        remove_custom_command(root)
    tree.write(editor_project)

    tree = ET.parse(runtime_project)
    root = tree.getroot()
    set_assembly_name(root, 'effect')
